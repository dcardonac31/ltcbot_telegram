...
while control_End_Bucle == 0:
    ...
    cursor.execute(list_sql_sentences[1])

    cursor2 = connDB.cursor()
    cursor2.execute('SELECT MAX(l.idbot) FROM public."LastBalanceByBot" l inner join bots b on l.idbot = b.idbot left join botsuspend s on l.idbot = s.idbotsuspend where l.idbot != 1 and s.idbotsuspend is null and l.balance < 0.00025 and l.idbot >= '+str(initialBot)+' and l.idbot <= '+str(endBot))
    ...
    print(blue+"Cont Bots-----> "+str(contbots))

    if contbots == 0:
        cursor.execute('SELECT l.idbot, b.phonenumber, b.apiid, b.apihash, l.balance FROM public."LastBalanceByBot" l inner join bots b on l.idbot = b.idbot left join botsuspend s on l.idbot = s.idbotsuspend where l.idbot != 1 and s.idbotsuspend is null and l.balance = 0 and l.idbot >= '+str(initialBot)+' and l.idbot <= '+str(endBot)+' order by l.balance desc')
        cursor2.execute('SELECT MAX(l.idbot) FROM public."LastBalanceByBot" l inner join bots b on l.idbot = b.idbot left join botsuspend s on l.idbot = s.idbotsuspend where l.idbot != 1 and s.idbotsuspend is null and l.balance < 0 and l.idbot >= '+str(initialBot)+' and l.idbot <= '+str(endBot))        
        for row in cursor:
            listBots.append(row)
            print(blue+"bot: "+str(row[0]) + " balance: "+str(row[4]))
            contbots = contbots + 1
